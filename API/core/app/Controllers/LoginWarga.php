...
{ 
    ...
    {           
        ...
        if($cekWarga != null){
            ...
            $cekAnggotaDasawisma    = $anggotaDasawisma->where('nik',$nik)->findAll();
            ...
            }else if($cekAnggotaDasawisma != null){
                $cekWargaPassword   = password_verify($pass,$cekWarga[0]['password']);                
                if($cekWargaPassword == true){
                    $token = rand(10,1000);
                    $updateToken = [
                    ...
                    $warga->update($cekWarga[0]['idWarga'],$updateToken);
                    $dataResponse = $data->getStrukturDasawismaDetail($nik); 
                    $response[] = [
                        'status' => 200,
                        'messages' => 'Login Berhasil',
                        'hakAkses' => 'dasawisma',
                    ...
                    $response = [
                        'status' => 400,
                        'messages' => 'Anda Bukan anggota dasawisma, password anda salah, Silahkan Coba Lagi'
                    ];
                    return $this->respond($response,400);  
                }
            }else{
